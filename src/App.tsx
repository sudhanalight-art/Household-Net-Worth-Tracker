...
const DashboardView = ({ 
  ...
  const { totalMonthlyIncome, totalMonthlyExpense } = useMemo(() => {
    ...
    plans.forEach((plan: Plan) => {
      ...
      if (typeStr.includes('income')) income += monthlyAmount;
      else if (typeStr.includes('expense')) expense += monthlyAmount;
    });
  ...
  const { trends, totals } = useMemo(() => {
    ...
    if (!historyData || historyData.length === 0) return { trends: result, totals: currentTotals };
  ...
  return (
    <div className="pb-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-4">
        <TrendBlock title="存款" typeKey="cash" data={trends.cash.data} assetKeys={Array.from(trends.cash.keys).sort((a: any, b: any) => a.localeCompare(b, 'zh-Hant'))} currentTotal={totals.cash} selectedOwner={selectedOwner} />
        <TrendBlock title="投資" typeKey="stock" data={trends.stock.data} assetKeys={Array.from(trends.stock.keys).sort((a: any, b: any) => a.localeCompare(b, 'zh-Hant'))} currentTotal={totals.stock} selectedOwner={selectedOwner} />
        <TrendBlock title="債務" typeKey="debt" data={trends.debt.data} assetKeys={Array.from(trends.debt.keys).sort((a: any, b: any) => a.localeCompare(b, 'zh-Hant'))} currentTotal={totals.debt} selectedOwner={selectedOwner} />
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {accounts.length === 0 ? (
            ...
            const displayName = selectedOwner === 'all' ? `${getOwnerDisplayName(normalizeOwner(acc.owner))} - ${acc.name}` : acc.name;
            return (
              <div key={acc.id} onClick={() => onEditAsset(acc)} className={`bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex justify-between items-center cursor-pointer hover:border-slate-300 transition-all group ${isDebt ? 'border-l-4 border-l-red-400' : ''}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full ${config.bgColor} flex items-center justify-center ${config.color}`}><config.icon size={18} /></div>
                  <div>
                    <div className="font-bold text-slate-700 text-sm">{displayName}</div>
                    <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1">
                      {selectedOwner !== 'all' && <span className={`px-1.5 py-0.5 rounded ${normalizeOwner(acc.owner) === 'husband' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>{getOwnerDisplayName(normalizeOwner(acc.owner))}</span>}
...
const EditModal = ({ isOpen, onClose, data, type, onSave }: any) => {
  ...
  useEffect(() => { 
    if (isOpen && data) setFormData({ ...data, type: data.type || (type === 'plan' ? 'expense' : 'cash'), currency: data.currency || 'TWD', owner: data.owner || 'husband', amount: data.amount || '', frequency: data.frequency || 'monthly' });
    else if (isOpen) setFormData({ owner: 'husband', type: type === 'plan' ? 'expense' : 'cash', currency: 'TWD', amount: '', frequency: 'monthly' });
  }, [isOpen, data, type]);
    ...
    <div className="fixed inset-0 z-[110] bg-slate-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center sm:p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl space-y-5 animate-in slide-in-from-bottom-10">
        <div className="flex justify-between items-center border-b pb-4">
          <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">{formData.id ? '編輯項目' : '新增項目'}<span className="text-xs font-normal bg-slate-100 px-2 py-1 rounded text-slate-500">{isPlan ? '固定收支' : '資產/負債'}</span></h3>
          <button onClick={onClose} className="bg-slate-100 p-2 rounded-full text-slate-500 hover:bg-slate-200"><X size={20}/></button>
        </div>
        <div className="space-y-4">
          <div className="flex gap-3">
             <div className="flex-1 space-y-1"><label className="text-xs font-bold text-slate-400">歸屬人</label><select className="w-full bg-slate-50 rounded-xl py-3 px-3 text-sm font-bold text-slate-700" value={normalizeOwner(formData.owner)} onChange={e => setFormData({...formData, owner: e.target.value})}><option value="husband">老公</option><option value="wife">老婆</option><option value="family">全家</option></select></div>
             <div className="flex-1 space-y-1"><label className="text-xs font-bold text-slate-400">類型</label><select className="w-full bg-slate-50 rounded-xl py-3 px-3 text-sm font-bold text-slate-700" value={normalizeType(formData.type)} onChange={e => setFormData({...formData, type: e.target.value})}>{isPlan ? (<><option value="expense">固定支出</option><option value="income">固定收入</option></>) : Object.entries(ASSET_TYPES).map(([k, v]: [string, any]) => (<option key={k} value={k}>{v.label}</option>))}</select></div>
          </div>
          <div className="space-y-1"><label className="text-xs font-bold text-slate-400">項目名稱</label><input type="text" className="w-full bg-slate-50 rounded-xl py-3 px-4 font-bold text-slate-800" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="例如：玉山銀行..." /></div>
          <div className="flex gap-3">
             <div className="w-1/3 space-y-1"><label className="text-xs font-bold text-slate-400">幣別</label><select className="w-full bg-slate-50 rounded-xl py-3 px-3 text-sm font-bold text-slate-700" value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value})}>{CURRENCY_LIST.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
             <div className="flex-1 space-y-1"><label className="text-xs font-bold text-slate-400">金額</label><input type="number" className="w-full bg-slate-50 rounded-xl py-3 px-4 font-bold text-slate-800" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} placeholder="0" /></div>
          </div>
        </div>
        <div className="pt-2"><button onClick={handleSave} className="w-full py-4 rounded-xl bg-slate-900 text-white font-bold shadow-lg flex justify-center items-center gap-2"><Save size={18}/> 儲存變更</button></div>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedUrl) { setApiUrl(savedUrl); setIsConfigured(true); refreshData(savedUrl); } 
  ...
  const refreshData = async (url: string) => {
    ...
    } catch (e) { console.error(e); setData({ assets: [], plans: [], history: [] }); } 
    finally { setLoading(false); }
  };

  const filteredAssets = data.assets.filter((a: Asset) => (selectedOwner === 'all' || normalizeOwner(a.owner) === selectedOwner) && Number(a.amount || 0) !== 0);
  const filteredPlans = data.plans.filter((p: Plan) => (selectedOwner === 'all' || normalizeOwner(p.owner) === selectedOwner) && Number(p.amount || 0) !== 0);
  const currentNetWorth = useMemo(() => {
    ...
    filteredAssets.forEach((a: Asset) => {
       ...
       const isDebt = normalizeType(a.type) === 'debt';
       if (isDebt) total -= val; else total += val;
    });
    return total;
  ...
  const handleSaveItem = async (formData: any) => {
    ...
    if (finalAmount === 0) newList = newList.filter((i: any) => i.id !== formData.id);
    else { const idx = newList.findIndex((i: any) => i.id === formData.id); if (idx >= 0) newList[idx] = formData; else newList.push({ ...formData, id: 'temp_'+Date.now() }); }
    
    const newHistory = [...data.history];
    if (newHistory.length > 0) {
        const lastIdx = newHistory.length - 1;
        const lastRecord: HistoryRecord = { ...newHistory[lastIdx], meta: { ...newHistory[lastIdx].meta } }; 
        ...
        lastRecord.meta[uniqueKey] = { name: formData.name, owner: ownerUpper, type: String(formData.type).toUpperCase(), currency: String(formData.currency).toUpperCase(), displayName: uniqueKey };
        newHistory[lastIdx] = lastRecord;
    }
    setData((prev: AppData) => ({ ...prev, [targetList]: newList, history: newHistory }));
    setEditModal({ ...editModal, isOpen: false });
    try { await fetch(apiUrl, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ action: editModal.type === 'plan' ? 'update_plan' : 'update_asset', date: new Date().toISOString().split('T')[0], ...formData, amount: finalAmount, owner: String(formData.owner).toUpperCase(), type: String(formData.type).toUpperCase(), currency: String(formData.currency).toUpperCase(), note: finalAmount === 0 ? 'DELETE' : '' }) }); } 
    catch(e) {}
  };
  ...
  if (!isConfigured) return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
      <div className="bg-white w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl">
        ...
        <button onClick={() => { const el = document.getElementById('urlInput') as HTMLInputElement; if(el?.value.includes('/exec')) { localStorage.setItem(STORAGE_KEY_API, el.value.trim()); setApiUrl(el.value.trim()); setIsConfigured(true); refreshData(el.value.trim()); } }} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold">連結系統</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen relative font-sans text-slate-900">
      ...
      <div className="bg-slate-900 pt-10 pb-6 rounded-b-[40px] relative overflow-hidden">
        <div className="relative z-20 px-6 flex justify-between items-start mb-6">
          <div className="flex-1"><div className="flex items-center gap-2 text-emerald-400 font-bold text-[10px] uppercase tracking-widest mb-1"><Building2 size={12}/> Family Finance</div><h1 onClick={() => setIsEditingTitle(true)} className="text-white font-bold text-lg cursor-pointer flex items-center gap-2 group">{appTitle} <Pencil size={12} className="opacity-0 group-hover:opacity-50 text-white"/></h1></div>
          <button onClick={() => setShowSettings(true)} className="bg-white/10 p-2 rounded-full text-white backdrop-blur-sm"><Settings size={18}/></button>
        </div>
      ...
      <div className="px-5 -mt-4 relative z-30">
        <div className="bg-white p-1 rounded-xl shadow-lg border border-slate-100 flex mb-6">{['all', 'husband', 'wife'].map(o => (<button key={o} onClick={() => setSelectedOwner(o)} className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all ${selectedOwner === o ? 'bg-slate-900 text-white shadow' : 'text-slate-400'}`}>{getOwnerDisplayName(o)}</button>))}</div>
        <DashboardView accounts={filteredAssets} plans={filteredPlans} rates={DEFAULT_RATES} selectedOwner={selectedOwner} displayCurrency={displayCurrency} onEditAsset={(item: any) => setEditModal({ isOpen: true, type: 'asset', data: item })} onEditPlan={(item: any) => setEditModal({ isOpen: true, type: 'plan', data: item })} historyData={data.history} />
        <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
          <button onClick={() => setEditModal({ isOpen: true, type: 'plan', data: null })} className="w-12 h-12 rounded-full bg-orange-500 text-white shadow-lg flex items-center justify-center hover:scale-110 active:scale-90" title="新增收支"><Banknote size={22}/></button>
          <button onClick={() => setEditModal({ isOpen: true, type: 'asset', data: null })} className="w-14 h-14 rounded-full bg-slate-900 text-white shadow-xl flex items-center justify-center hover:scale-110 active:scale-90" title="新增資產"><Landmark size={24}/></button>
        </div>
      </div>
