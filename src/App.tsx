...
import { 
  ...
  CreditCard, Landmark, 
  Wallet, Pencil, TrendingUp, X, ChevronRight,
...
const FREQUENCY_OPTS: Record<string, { label: string, divisor: number }> = {
          ...
          <div className="grid grid-cols-5 gap-2 mb-6">
            {CURRENCY_LIST.map(curr => (
              <button 
                ...
                className={`text-[10px] font-bold py-2 rounded-lg transition-all ${currentCurrency === curr ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
              >
                {curr}
              </button>
            ))}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1">
                      {selectedOwner !== 'all' && (
                        <span className={`px-1.5 py-0.5 rounded ${normalizeOwner(acc.owner) === 'husband' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
...
export default function App() {
  ...
  const [data, setData] = useState<any>({ assets: [], plans: [], history: [] });
  ...
  const handleSaveItem = async (formData: any) => {
    ...
    const targetList = (editModal.type === 'plan' ? 'plans' : 'assets');
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="bg-white w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl">
          ...
          <input 
            ...
            className="w-full bg-slate-50 border p-4 rounded-xl mb-4 text-xs outline-none"
          />
          <button 
            onClick={() => {
  ...
  const currentNetWorth = filteredAssets.reduce((total: number, a: any) => {
    ...
    return isDebt ? total - val : total + val;
  }, 0);
    ...
    <div className="max-w-md mx-auto bg-white min-h-screen relative font-sans text-slate-900">
      ...
      <div className="px-5 -mt-4 relative z-30">
        ...
        <DashboardView 
          ...
          plans={data.plans.filter((p: any) => (selectedOwner === 'all' || normalizeOwner(p.owner) === selectedOwner) && Number(p.amount || 0) !== 0)} 
          rates={DEFAULT_RATES}
          selectedOwner={selectedOwner}
